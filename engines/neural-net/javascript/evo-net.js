...
/* ****************************************************************************
; Object: evoNet
; Description: evolutionary neural network.
...
export default function EvoNet(numInternalLayers, numInputs, numOutput) {
  ...
  this.weights = [...Array(this.numNetLayers)].map(() =>
    [...Array(this.NumNeuronsPerLayer)].map(() =>
      [...Array(this.numWeightsPerNeuron)].map(() =>
        Random.randomFromInterval(-0.15, 0.15),
      ),
    ),
  );

  /* the bias allows for some independence from inputs */
  this.bias = [...Array(this.numNetLayers)].map(() =>
    [...Array(this.NumNeuronsPerLayer)].map(() =>
      Random.randomFromInterval(-0.15, 0.15),
    ),
  );

  this.outputs = [...Array(this.numNetLayers)].map(() =>
    [...Array(this.NumNeuronsPerLayer)].fill(0),
  );
}
...
EvoNet.prototype.run = function (inputData) {
  ...
  for (n = 0; n < this.NumNeuronsPerLayer; n++) {
    ...
    for (w = 0; w < this.numInputs; w++) {
      ...
      this.outputs[INPUT_LAYER][n] +=
        inputData[w] * this.weights[INPUT_LAYER][n][w];
    } /* weights */
  ...
  for (
    l = 1;
    /* after input layer... */ l < this.numNetLayers - 1;
    /* -1 means: do not exec output layer */ l++
  ) {
    for (n = 0; n < this.NumNeuronsPerLayer; n++) {
      this.outputs[l][n] = 0;
      ...
      for (w = 0; w < this.numWeightsPerNeuron; w++) {
        this.outputs[l][n] += this.outputs[l - 1][w] * this.weights[l][n][w];
      } /* weights */
      ...
      */
      ...
      /* sigmoid func */
      this.outputs[l][n] = 1.0 / (1.0 + Math.exp(-this.outputs[l][n]));
    } /* neurons */
  } /* layers */
  ...
  for (n = 0; n < this.NumNeuronsPerLayer; n++) {
    ...
    for (w = 0; w < this.numWeightsPerNeuron; w++) {
      ...
      */
      ...
      this.outputs[this.outputLayerIndex][n] += Math.pow(
        this.outputs[this.outputLayerIndex - 1][w],
...
EvoNet.clone = function () {
  let newNet = new EvoNet(
    this.numInternalLayers,
    this.numInputs,
    this.numOutput,
  );
